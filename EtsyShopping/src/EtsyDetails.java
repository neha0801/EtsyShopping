...
//						+ "<label class=\"control-label col-sm-2\" for=\"item" + itemId + "\">Quantity:</label>"
//						+ "<input type=\"text\" name=\"item" + itemId + "\" id=\"item" + itemId + "\" value=\"" + cart.get(cartIndex).getPquantity() + "\">"
//						+ "<button type=\"submit\" class=\"btn btn-default\" name=\"singleUpd\" id=\"singleUpd\">Add to Cart</button></form>";
//			} else {
				details += "<div class=\"container\"><br /><form class=\"form-horizontal\" role=\"form\" name=\"singleProd\" id=\"singleProd\" action=\"EtsyDetails\" method=\"post\">"
						+ "<input type=\"hidden\" name=\"itemId\" value=\"" + itemId + "\">"
						+ "<label class=\"control-label col-sm-2\" for=\"item" + itemId + "\">Quantity:</label>"
						+ "<input type=\"text\" name=\"item" + itemId + "\" id=\"item" + itemId + "\">"
						+ "<button type=\"submit\" class=\"btn btn-default\" name=\"singleUpd\" id=\"singleUpd\">Add to Cart</button></form>";
//			}
		}

		request.setAttribute("details", details);
		getServletContext().getRequestDispatcher("/EtsyItemDetail.jsp").forward(request, response);
	}
