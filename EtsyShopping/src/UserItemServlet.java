...
public class UserItemServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		List<Etsyitem> myProducts = EtsyitemDB.selectByUserName(user.getName());
		
		
		if (myProducts != null && !myProducts.isEmpty()) {
			sellingList += "<div class=\"container\"><br />";
			sellingList += "<table class=\"table table-striped\">"
					+ "<thead><tr><th>Picture</th><th>Name</th><th>Description</th><th>Price</th>"
					+ "<th>Shipping Cost</th><th>Instock</th><th></th></tr></thead><tbody>";

			for (int i = 0; i < myProducts.size(); i++) {
				sellingList += "<tr><td><img src=\""
						+ myProducts.get(i).getItenPicture()
						+ "\" alt=\"No image\" width=\"42\" height=\"42\"></td><td>"
						+ myProducts.get(i).getItemName()
						+ "</td><td>"
						+ myProducts.get(i).getItemDescription()
						+ "</td><td>"
						+ formattedPrice(myProducts.get(i).getItemPrice())
						+ "</td><td>"
						+ formattedPrice(myProducts.get(i)
								.getItemShippingcost())
						+ "</td><td>"
						+ myProducts.get(i).getItemInstock()
						+ "</td><td><a href=\"UserItemServlet?itemId="
						+ myProducts.get(i).getItemId()
						+ "\" class=\"btn btn-info\" role=\"button\">Edit</a></td></tr>";
			}
		...
		if (request.getParameter("itemId") != null) {
			...
			getServletContext().getRequestDispatcher("/UserItem.jsp").forward(
					request, response);
		}
	}
	...
	protected void doPost(HttpServletRequest request,
		...
		editproductform = "<div class=\"container\">"
				...
				+ "<div class=\"col-sm-10\">"
				+ "<input type=\"text\" class=\"form-control\" name=\"shipping\" id=\"shipping\" value=\""
				+ currentItem.getItemShippingcost()
				+ "\">"
				+ "</div>"
				+ "<div class=\"form-group\">"
				...
				+ "<input type=\"radio\" name=\"instock\" id=\"instock\" value=\"1\" checked=\"checked\">Available"
				+ "</label>"
				+ "<label class=\"radio-inline\">"
